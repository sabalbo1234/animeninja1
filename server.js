...
if (require.main === module) {
  // Cambiar el log para que no quede fijo en 127.0.0.1
  serveHTTP(builder.getInterface(), { port: ADDON_PORT });
  console.log(`Addon running at http://0.0.0.0:${ADDON_PORT}/manifest.json`);

  const imgServer = http.createServer(async (req, res) => {
    try {
      // Aquí también: de 127.0.0.1 -> 0.0.0.0
      const parsed = new URL(req.url, `http://0.0.0.0:${IMG_PROXY_PORT}`);
      const parts = parsed.pathname.split("/").filter(Boolean);
      if (parts.length < 2 || parts[0] !== "img") {
      ...
      res.writeHead(302, { Location: "https://stremio.com/website/stremio-logo-small.png" });
      res.end();
    }
  });

  // Aquí ya lo tienes bien en 0.0.0.0
  imgServer.listen(IMG_PROXY_PORT, "0.0.0.0", () => {
    console.log(`Image proxy at http://0.0.0.0:${IMG_PROXY_PORT}/img/<base64>`);
  });
} else {
